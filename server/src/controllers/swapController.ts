...
export async function createSwapRequest(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    if (mySlot.ownerId !== requesterId) return res.status(403).json({ message: 'Not your slot' });
    if (mySlot.status !== 'SWAPPABLE' || theirSlot.status !== 'SWAPPABLE') {
      return res.status(400).json({ message: 'Both slots must be SWAPPABLE' });
    }
    if (theirSlot.ownerId === requesterId) return res.status(400).json({ message: 'Cannot swap with yourself' });
...
export async function respondToSwap(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    if (reqRecord.responderId !== responderId) return res.status(403).json({ message: 'Forbidden' });
    ...
    if (!mySlot || !theirSlot) return res.status(404).json({ message: 'Slots missing' });
